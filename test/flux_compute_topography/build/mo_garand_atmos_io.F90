...
contains
  ...
  subroutine read_atmos(fileName,                          &
    ...
    character(len=3), dimension(ngases), parameter :: &
    ...
    do igas = 1, ngases
      ...
      if(var_exists(ncid, 'vmr_' // trim(gas_name))) &
        call stop_on_err(gas_concs%set_vmr(gas_name, read_field(ncid, 'vmr_' // trim(gas_name), ncol, nlay)))
    end do 
